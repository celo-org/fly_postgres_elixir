defmodule Fly.Postgres.Elixir.Telemetry do
  @moduledoc """
    Common telemetry module
  """

  alias __MODULE__
  @tag :fly_postgres_elixir

  @doc false
  def start(name, meta \\ %{}, measurements \\ %{}) do
    time = System.monotonic_time()
    measures = Map.put(measurements, :system_time, time)
    :telemetry.execute([@tag, name, :start], measures, meta)
    time
  end
  ...
  def exception(event, start_time, kind, reason, stack, meta \\ %{}, extra_measurements \\ %{}) do
    ...
    :telemetry.execute([@tag, event, :exception], measurements, meta)
  end

  @doc false
  def event(name, metrics, meta \\ %{}) do
    :telemetry.execute([@tag, name], metrics, meta)
  end
